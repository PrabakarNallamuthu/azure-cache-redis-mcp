...
func Redis_exportdataHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/export%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateRedis_exportdataTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_export",
		mcp.WithDescription("Export data from the redis cache to blobs in a container."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Parameters for Redis export operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_exportdataHandler(cfg),
	}
}
