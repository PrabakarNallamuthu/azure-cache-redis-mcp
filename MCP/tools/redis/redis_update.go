...
func Redis_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRedis_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name",
		mcp.WithDescription("Update an existing Redis cache."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Parameters supplied to the Update Redis operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_updateHandler(cfg),
	}
}
