...
func Patchschedules_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePatchschedules_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_patchSchedules_default",
		mcp.WithDescription("Gets the patching schedule of a redis cache (requires Premium SKU)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Patchschedules_getHandler(cfg),
	}
}
