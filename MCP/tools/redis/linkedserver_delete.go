...
func Linkedserver_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateLinkedserver_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_linkedServers_linkedServerName",
		mcp.WithDescription("Deletes the linked server from a redis cache (requires Premium SKU)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Linkedserver_deleteHandler(cfg),
	}
}
