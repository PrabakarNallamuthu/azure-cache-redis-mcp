...
func Linkedserver_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/linkedServers%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateLinkedserver_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_linkedServers",
		mcp.WithDescription("Gets the list of linked servers associated with this redis cache (requires Premium SKU)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Linkedserver_listHandler(cfg),
	}
}
