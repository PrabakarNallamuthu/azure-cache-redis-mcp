...
func Redis_listbyresourcegroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis%s", cfg.BaseURL, resourceGroupName, subscriptionId, queryString)
...
func CreateRedis_listbyresourcegroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis",
		mcp.WithDescription("Lists all Redis caches in a resource group."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_listbyresourcegroupHandler(cfg),
	}
}
