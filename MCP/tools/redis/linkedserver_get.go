...
func Linkedserver_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateLinkedserver_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_linkedServers_linkedServerName",
		mcp.WithDescription("Gets the detailed information about a linked server of a redis cache (requires Premium SKU)."),
		...
		mcp.WithString("linkedServerName", mcp.Required(), mcp.Description("The name of the linked server.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Linkedserver_getHandler(cfg),
	}
}
