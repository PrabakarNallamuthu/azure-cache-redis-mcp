...
func Firewallrules_listbyredisresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/firewallRules%s", cfg.BaseURL, subscriptionId, resourceGroupName, cacheName, queryString)
...
func CreateFirewallrules_listbyredisresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_cacheName_firewallRules",
		mcp.WithDescription("Gets all firewall rules in the specified redis cache."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Firewallrules_listbyredisresourceHandler(cfg),
	}
}
