...
func Redis_importdataHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/import%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateRedis_importdataTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_import",
		mcp.WithDescription("Import data into Redis cache."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Parameters for Redis import operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_importdataHandler(cfg),
	}
}
