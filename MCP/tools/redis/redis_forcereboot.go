...
func Redis_forcerebootHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/forceReboot%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateRedis_forcerebootTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_forceReboot",
		mcp.WithDescription("Reboot specified Redis node(s). This operation requires write permission to the cache resource. There can be potential data loss."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Specifies which Redis node(s) to reboot.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_forcerebootHandler(cfg),
	}
}
