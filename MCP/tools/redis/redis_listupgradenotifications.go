...
func Redis_listupgradenotificationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/listUpgradeNotifications%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateRedis_listupgradenotificationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_listUpgradeNotifications",
		mcp.WithDescription("Gets any upgrade notifications for a Redis cache."),
		...
		mcp.WithString("history", mcp.Required(), mcp.Description("how many minutes in past to look for upgrade notifications")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Redis_listupgradenotificationsHandler(cfg),
	}
}
