...
func Firewallrules_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateFirewallrules_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_cacheName_firewallRules_ruleName",
		mcp.WithDescription("Deletes a single firewall rule in a specified redis cache."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Firewallrules_deleteHandler(cfg),
	}
}
