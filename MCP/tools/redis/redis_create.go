...
func Redis_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRedis_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name",
		mcp.WithDescription("Create or replace (overwrite/recreate, with potential downtime) an existing Redis cache."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Parameters supplied to the Create Redis operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_createHandler(cfg),
	}
}
