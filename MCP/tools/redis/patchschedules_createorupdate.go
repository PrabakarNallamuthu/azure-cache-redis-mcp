...
func Patchschedules_createorupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		defaultVal, ok := args["default"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: default"), nil
		}
		default, ok := defaultVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: default"), nil
		}
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/patchSchedules/%s%s", cfg.BaseURL, resourceGroupName, name, default, subscriptionId, queryString)
...
func CreatePatchschedules_createorupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_patchSchedules_default",
		mcp.WithDescription("Create or replace the patching schedule for Redis cache (requires Premium SKU)."),
		...
		mcp.WithString("default", mcp.Required(), mcp.Description("Default string modeled as parameter for auto generation to work correctly.")),
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Parameters to set the patching schedule for Redis cache.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Patchschedules_createorupdateHandler(cfg),
	}
}
