...
func Patchschedules_listbyredisresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/patchSchedules%s", cfg.BaseURL, subscriptionId, resourceGroupName, cacheName, queryString)
...
func CreatePatchschedules_listbyredisresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_cacheName_patchSchedules",
		mcp.WithDescription("Gets all patch schedules in the specified redis cache (there is only one)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Patchschedules_listbyredisresourceHandler(cfg),
	}
}
