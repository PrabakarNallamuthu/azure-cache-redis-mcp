...
func Firewallrules_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateFirewallrules_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_cacheName_firewallRules_ruleName",
		mcp.WithDescription("Gets a single firewall rule in a specified redis cache."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Firewallrules_getHandler(cfg),
	}
}
