...
func Redis_regeneratekeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Cache/Redis/%s/regenerateKey%s", cfg.BaseURL, resourceGroupName, name, subscriptionId, queryString)
...
func CreateRedis_regeneratekeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.Cache_Redis_name_regenerateKey",
		mcp.WithDescription("Regenerate Redis cache's access keys. This operation requires write permission to the cache resource."),
		...
		mcp.WithString("parameters", mcp.Required(), mcp.Description("Specifies which key to regenerate.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Redis_regeneratekeyHandler(cfg),
	}
}
